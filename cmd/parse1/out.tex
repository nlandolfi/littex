...
If {$A$} and {$B$} are sets and if every element of {$A$} is an element of {$B$}, we say that {$A$} is a _subset_ of {$B$}, or $B$ _includes_ {$A$}, and we write  or  

The wording of the definition implies that each set must be considered to be included in itself ({$A \subset A$}); this fact is described by saying that set inclusion is _reflexive_. (Note that, in the same sense of the word, equality also is reflexive.) 

If {$A$} and {$B$} are sets such that {$A \subset B$} and {$A \neq B$}, the word _proper_ is used (proper subset, proper inclusion). 

If {$A$}, {$B$}, and {$C$} are sets such that {$A \subset B$} and {$B \subset C$}, then {$A \subset C$}; this fact is described by saying that set inclusion is _transitive_. 
...
This fact is described by saying that set inclusion is _antisymmetric_ 

(In this respect, set inclusion behaves differently from equality. 

Equality is _symmetric_, in the sense that if {$A = B$}, then necessarily {$B = A$}.) 
